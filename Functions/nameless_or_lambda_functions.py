# filtering can be applied also with a shortcut syntax, and this is called lambda function
# filter(lambda_function, iterator object)
# the drawback is the fact that being a nameless function, the lambda construction can not be appealed later
employees = [
...
new_object_age_not_job = filter(
    lambda employee: employee["age"] > 30  and employee["job"] != "Project Manager",
    employees
)

print(f"The object obtained through filtering of lambda is \t -> {new_object_age_not_job}")

new_list_age_not_job = list(new_object_age_not_job)
print(f"The list obtained is \t -> {new_list_age_not_job}")


new_object_age_only = filter(lambda employee: employee["age"] > 35, employees)
print(f"The object is \t -> {new_object_age_only}")

new_list_age_only = list(new_object_age_only)
print(f"The list obtained is {new_list_age_only}")