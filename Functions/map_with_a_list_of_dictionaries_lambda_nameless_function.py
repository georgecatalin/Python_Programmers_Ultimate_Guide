...
# when it refers to a single parameter , parentheses are not compulsory
# the lambda function has to return a dictionary object,hence the ** kwargs operator has to be utilized to fill in the entire dictionary
new_object_convert = map(lambda employee:
                         {
                             **employee,
                             "job" : employee["job"].upper() if employee["job"] != "Project Manager"  else employee["job"]
                          }, employees)
